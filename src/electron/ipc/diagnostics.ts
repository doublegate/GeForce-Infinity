...
export function registerDiagnosticsHandlers({
    ...
    ipcMain.handle("get-diagnostics-report", async (): Promise<DiagnosticsReport> => {
        console.log("[GeForce Infinity] IPC: get-diagnostics-report");
        try {
            return await generateDiagnosticsReport(mainWindow.webContents);
        } catch (error) {
            console.error("[GeForce Infinity] Error generating diagnostics report:", error);
            throw error;
        }
    });

    // Run comprehensive codec tests
    ipcMain.handle("run-codec-tests", async (): Promise<DiagnosticsTestResults> => {
    ...
    ipcMain.handle("get-codec-capabilities", async (): Promise<CodecCapabilities> => {
        console.log("[GeForce Infinity] IPC: get-codec-capabilities");
        try {
            return await getCodecCapabilities(mainWindow.webContents);
        } catch (error) {
            console.error("[GeForce Infinity] Error getting codec capabilities:", error);
            throw error;
        }
    });
    ...
    ipcMain.handle(
        ...
        async (): Promise<{
            ...
            try {
                ...
                console.error("[GeForce Infinity] Error getting diagnostics summary:", error);
                throw error;
            }
        }
    );
    ...
    console.log("[GeForce Infinity] Diagnostics IPC handlers registered successfully");
}
