...
async function checkVaapiAvailable(): Promise<boolean> {
    ...
    try {
        ...
        if (stdout) {
            const hasVaapi = stdout.includes("VA-API version") || stdout.includes("vainfo:");
            console.log("[GeForce Infinity] VAAPI check:", hasVaapi ? "Available" : "Not Available");
            return hasVaapi;
        }
        ...
        console.log("[GeForce Infinity] DRI render nodes:", hasRenderNodes ? "Found" : "Not Found");
...
async function checkHEVCExtensions(): Promise<boolean> {
    ...
    try {
        ...
        console.log("[GeForce Infinity] HEVC Extensions:", hasExtensions ? "Installed" : "Not Installed");
...
function checkVideoToolbox(): boolean {
    ...
    try {
        ...
        console.log("[GeForce Infinity] VideoToolbox:", available ? "Available" : "Not Available");
        return available;
    } catch {
 ...
 */
export async function getDriverInfo(): Promise<{ driver: string; version: string }> {
    if (process.platform === "linux") {
 ...
 */
async function getLinuxDriverInfo(): Promise<{ driver: string; version: string }> {
    try {
        // Check for NVIDIA
        ...
        const glxResult = safeExecSync("glxinfo 2>/dev/null | grep 'OpenGL version'");
        if (glxResult) {
 ...
 * @returns Promise<{driver: string, version: string}> - Driver info
 */
async function getWindowsDriverInfo(): Promise<{ driver: string; version: string }> {
    try {
        const stdout = await safeExec("powershell", [
            "-Command",
...
export async function getPlatformRecommendations(): Promise<string[]> {
    ...
    if (process.platform === "linux") {
        ...
        if (!vaapiAvailable) {
            recommendations.push("Install VAAPI drivers for hardware-accelerated video decoding");
            recommendations.push("For Intel: install intel-media-driver or libva-intel-driver");
            recommendations.push("For AMD: install libva-mesa-driver");
            recommendations.push("For NVIDIA: install libva-nvidia-driver (experimental)");
        }
        ...
        if (!hevcInstalled) {
            recommendations.push("Install HEVC Video Extensions from Microsoft Store for HEVC support");
            recommendations.push("Free version may be available from your GPU manufacturer");
        }
    } else if (process.platform === "darwin") {
        recommendations.push("macOS uses VideoToolbox for hardware acceleration (built-in)");
        recommendations.push("Ensure you are running macOS Big Sur (11.0) or later for optimal codec support");
    }
...
export async function getPlatformWarnings(): Promise<string[]> {
    ...
    if (process.platform === "linux") {
        ...
        if (sessionType === "wayland") {
            warnings.push("Running on Wayland - some features may behave differently");
        }

        // Check NVIDIA driver version
        const nvidiaVersion = safeExecSync("cat /sys/module/nvidia/version 2>/dev/null").trim();
        if (nvidiaVersion) {
            const version = parseInt(nvidiaVersion.split(".")[0]);
            if (version < 515) {
                warnings.push("NVIDIA driver version is older than 515 - consider updating for best AV1 support");
            }
        }
    }
