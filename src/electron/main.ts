...
import {
  setupSessionRequestInterception,
  setupPlatformHeaderOverride,
  patchFetchForSessionRequest,
...
function setupWindowEvents(mainWindow: BrowserWindow) {
  mainWindow.webContents.on("did-finish-load", async () => {
    ...
    await patchFetchForSessionRequest(mainWindow, getConfig);
  });
  ...
  mainWindow.on("page-title-updated", (event, title) => {
    ...
    if (title === "Game ending in 60s") {
      ...
      if (config.inactivityNotification === true && config.autofocus === true) {
        mainWindow.maximize();
      }
    }
    let gameName = title
  ...
  // Network interception (session requests, platform headers) handled by ./network/interceptor.js
  // Called via setupSessionRequestInterception and setupPlatformHeaderOverride in app.whenReady()
...
app.whenReady().then(async () => {
  ...
  // Setup network interception for resolution override and platform headers
  setupSessionRequestInterception(getConfig);
  setupPlatformHeaderOverride();
